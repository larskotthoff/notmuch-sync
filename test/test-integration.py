...
def sync(shell, local_conf, remote_conf, verbose=False, delete=False, mbsync=False):
    args = ["./notmuch-sync-linux", "--remote-cmd", f"bash -c 'NOTMUCH_CONFIG={remote_conf} ./src/notmuch-sync {"--delete" if delete else ""} {"--mbsync" if mbsync else ""}'"]
    if verbose:
        args.append("--verbose")
    if delete:
        args.append("--delete")
...
def test_sync_tags_files_verbose(shell):
    with TemporaryDirectory() as local:
        with TemporaryDirectory() as remote:
            ...
            assert 'Changes synced.' in out[7]
            ...
            assert '9426/4616 bytes received from/sent to remote.' in out[23]
